...
class FriendsModel{
    ...
    function GetChapters($season = null){
        if ($season !== "all") {
            $sentencia = $this->db->prepare("SELECT * FROM chapter INNER JOIN season ON chapter.id_season = season.id AND season.season = ?");
            $sentencia->execute(array($season));
        }
        else {
            $sentencia = $this->db->prepare("SELECT * FROM chapter");
            $sentencia->execute();
        }
